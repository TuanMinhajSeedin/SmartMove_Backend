...
def convert_json_to_schedule_format(json_data: Dict, optional_properties: Dict = None) -> List[Dict]:
    ...
    if "extracted_data" in json_data:
        ...
        for table in tables:
            ...
            # Remove unwanted columns (and their values) such as
            # "Passenger Service Permit Number" and "Bus Number"
            if headers:
                filtered_indices = []
                for idx, hdr in enumerate(headers):
                    ...
                    if 'passenger service permit' in hl or 'bus number' in hl:
                        continue
                    filtered_indices.append(idx)

                # If we removed any columns, rebuild headers and rows
                if len(filtered_indices) != len(headers):
                    headers = [headers[i] for i in filtered_indices]
                    new_rows = []
                    for row in rows:
                        new_row = []
            ...
            for idx, header in enumerate(headers):
                ...
                if 'route type' in header_lower:
                    ...
                    continue
                if 'service' in header_lower or 'service_type' in header_lower:
                    service_type_index = idx
                    continue
                ...
                for row in rows:
                    if len(row) > max(from_idx, to_idx):
                        ...
                        if from_time_str and to_time_str:
                            ...
                            # Row-level route type column (if present)
                            ...
                            # Row-level service type column (if present)
                            if service_type_index is not None and service_type_index < len(row):
                                service_type_value = row[service_type_index]
                                if service_type_value is not None:
                                    service_type_value = str(service_type_value).strip()
                                    if service_type_value and service_type_value.lower() not in ['', 'nan', 'none']:
                                        schedule_item["service_type"] = service_type_value
